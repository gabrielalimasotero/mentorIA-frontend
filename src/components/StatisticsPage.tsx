...
const StatisticsPage = () => {
  ...
  const loadStatistics = () => {
    ...
    // Verificar se o diagnóstico foi completado
    ...
    // Só processar dados se o diagnóstico foi completado
    if (hasCompletedDiagnostic) {
      // Processar progresso do diagnóstico
    ...
    const topicsProgress = Object.entries(SUBTOPICS_VALIDOS).map(([topic, subtopics]) => {
      if (hasCompletedDiagnostic) {
        ...
        // Mostrar 0 para todos os tópicos quando não há diagnóstico
        return {
          topic,
    ...
    // Atividade recente - mostrar vazio se não há atividade
    ...
    setStatistics({
      ...
      totalStudyTime: hasCompletedDiagnostic ? Math.floor(totalQuestions * 2.5) : 0, // 2.5 minutos por questão
      completedTests,
